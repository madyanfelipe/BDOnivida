...
DROP TABLE IF EXISTS tab_planeac_cumplim_activ;		-- FORM 10 Incluye tab_planeac_cumplim_activ tab_actividades_plan tab_plan_anual tab_estructura_sgsst 
DROP TABLE IF EXISTS tab_actividades_plan;          -- FORM 10 Incluye tab_planeac_cumplim_activ tab_actividades_plan tab_plan_anual tab_estructura_sgsst 
DROP TABLE IF EXISTS tab_plan_anual;                -- FORM 10 Incluye tab_planeac_cumplim_activ tab_actividades_plan tab_plan_anual tab_estructura_sgsst 
...
DROP TABLE IF EXISTS tab_estructura_sgsst;          -- FORM 10 Incluye tab_cumpl_actividades_plan tab_actividades_plan tab_plan_anual tab_estructura_sgsst 
...
(
    ...
    entidad                 INTEGER   NOT NULL,     -- 1=Empresa 2=Empleado 3=Proveedor
...
(
    ...
    id_cargo                 VARCHAR          NOT NULL, --relacion con la tabla de cargos
    id_nivel_estudio         VARCHAR          NOT NULL, --relacion con la tabla de niveles de estudio 
    tipo_documento           VARCHAR        NOT NULL, --Almacena tipo de documento (1: Tarjeta de Identidad / 2: Cédula de ciudadania / 3: Pasaporte )
    identificacion           SMALLINT         NOT NULL, --Número de documento
    ...
    telefono_contacto        VARCHAR NOT NULL,         --telefono contacto
...
(
    ...
    comite                  INTEGER     NOT NULL, --1=COPASST, 2=COCOLA, 3=Brigada de emergencia
    perfil                  INTEGER     NOT NULL, --Se almacena el rol o perfil que desempeñara el empleado dentro del comite 1=Presidente, 2=Suplente del Presidente, 3=Secretario, 4=Suplente del Secretario, 5=miembro
    observaciones             VARCHAR,            --Se registra observaciones en caso que aplique
...
COMMENT ON COLUMN tab_empleados_comites.comite IS '1=COPASST, 2=COCOLA, 3=Brigada de emergencia';
...
COMMENT ON COLUMN tab_empleados_comites.observaciones IS 'Se registra observaciones en caso que aplique';
...
COMMENT ON COLUMN tab_actas.comite IS '1=COPASST, 2=COCOLA, 3=Brigada de emergencia';
...
COMMENT ON COLUMN tab_quejas.id_empleado IS 'id del empleado que pone la queja ';
...
-- en esta tabla se incluyen los planes anuales de capacitacion de los comites y el plan de trabajo anual
-- Tabla de plan anual
CREATE TABLE tab_plan_anual
(
    id                      SERIAL,
    tipo_plan               SMALLINT         NOT NULL, --1=plan de trabajo 2=plan de capacit sgsst 3=plan capacit BE 4=plancapacit gestion humana 
    ...
    activ_planeadas         SMALLINT,  --Cantidad de actividades planeadas estos campos son calculados
...
COMMENT ON TABLE tab_plan_anual IS 'Contiene los registros  de los planes anuales de trabajo y de capacitaciones ';
COMMENT ON COLUMN tab_plan_anual.año IS 'El año con el tipo de plan tambien podría ser la llave primaria ya que no se puede repetir';
...
(
    ...
    equipo_responsable      VARCHAR        NOT NULL, --Se relaciona con la tabla de equipo responsable 
...
COMMENT ON COLUMN tab_actividades_plan.equipo_responsable IS 'Se digita, pueden ser responsables varios equipos';
COMMENT ON COLUMN tab_actividades_plan.periodicidad IS 'En texto ingresa el si es quincenal, mensual ...';

-- ej se planea la actividad 
-- Diseñar  y hacer seguimiento al  plan de trabajo con intermediación de la dministradora de riesgos  para la ejecución durante el 2021
-- esta actividad es mensual por lo tanto se deben crear 12 registros con fecha en que se deben realizar
-- esos doce registros se editan con la fecha en que se realizaron
-- con estos datos se crea la consulta con las actividades planeadas y realizadas del mes. 
-- se puede cumplir con una parte de la actividad, ejemplo 0.5
-- se va a guardar la fecha planeada y no la fecha en que se cumple, solo se edita la cumplimiento de la actividad
-- Tabla planeacion y cumplimiento de actividades del plan anual
CREATE TABLE tab_planeac_cumplim_activ
(
    id                      SERIAL,
    id_actividad            BIGINT         NOT NULL,--Relacion con la tabla de actividades
    fecha_planeada          INTEGER        NOT NULL,--fecha en que se debe cumplir la actividad
    cumplimiento            DECIMAL(3,2), -- cumpliento de la actividad ej 0.5 ó 1 1=100% de la actividad
    fecha_creacion          TIMESTAMP DEFAULT NOW(),  --Fecha de registro de la actividad en el sistema
    activo                  BOOLEAN   DEFAULT TRUE NOT NULL,  --Estado (True: Activo / False:Inactivo)
    PRIMARY KEY(id),
    FOREIGN KEY(id_actividad) REFERENCES tab_actividades_plan(id)
);
COMMENT ON TABLE tab_planeac_cumplim_activ IS 'Contiene los registros de la planeacion y cumplimiento de las actividades del plan';
...
(
    id                      SERIAL,
    id_plan                 BIGINT,                     -- id del plan anual
    area                    VARCHAR,
    tema                    TEXT          NOT NULL,
...
(
    ...
    fecha_planeada          TIMESTAMP,                  --Fecha en que se deben realizar las actividades de capacitacion
    ejecutado               BOOLEAN,                    --(true:Ejecutado / false:Pendiente)
...
COMMENT ON COLUMN tab_planeacion_capacitacion.ejecutado  IS 'En que estado se encuentra la actividad actividad (false: Pendiente - true: Ejecutada)';
